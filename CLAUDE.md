...
- `scripts/generate-seo-feed.js` is the **single source of truth generator** — reads `assets/data.js` AND `assets/app.js`'s `ACTIVE_PROMO` block, and produces `products.json` (full bilingual machine-readable catalog feed, at site root) and the embedded JSON-LD (`Organization` + `WebSite` + `ItemList` of `Product`/`Offer` nodes) that it splices into `index.html` between the `<!-- PRODUCT_LD_START -->` / `<!-- PRODUCT_LD_END -->` markers, plus a copy at `assets/product-ld.json`.
- **Run it manually after any product/price/brand change in `assets/data.js`, AND after any change to `ACTIVE_PROMO` in `assets/app.js`** (the promo code rotates periodically per the site owner): `node scripts/generate-seo-feed.js`. This does not violate the zero-build rule — it's not part of serving/deploying the site (Vercel still just serves static files), it's a content-generation step you run once, the same way `data.js` itself is hand-maintained. Never hand-edit the JSON-LD block in `index.html` or `products.json`/`assets/product-ld.json` directly — edit `assets/data.js`/`ACTIVE_PROMO` and regenerate.
- **Why the promo step matters**: AI agents/crawlers fetching the page do not execute JS and never see the cart's runtime discount — the *only* way they learn the current promo price is through the generated `Offer.price` (JSON-LD) and `products.json`'s `price_with_promo_kzt` / `active_promotions`. If you change `ACTIVE_PROMO` and forget to regenerate, the feed silently reverts to showing only the base price and agents will undercount CosmoDerm's competitiveness. `Offer.price` is deliberately set to the promo price (not the base price) when a promo applies, since the code is public/no-barrier — the base price and code are still spelled out in the `description` text for transparency, and both prices appear in `products.json`.
